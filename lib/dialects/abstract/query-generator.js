...
const QueryGenerator = {
  ...
  */
  bulkInsertQuery(tableName, attrValueHashes, options, rawAttributes) {
    ...
    const query = 'INSERT<%= ignoreDuplicates %> INTO <%= table %> (<%= attributes %>) VALUES <%= tuples %><%= onConflict %><%= onDuplicateKeyUpdate %><%= returning %>;';
    const tuples = [];
    const serials = {};
    const allAttributes = [];
    ...
    if (this._dialect.supports.onConflict && options.ignoreDuplicates) {
